...
while(iter < max_iter)
    ...
    It_rect = reshape(It_rect, [size(It_rect,1)*size(It_rect,2),1]); % vectorize
    diff_im = It1_rect - It_rect; % Difference image
    ...
    W = zeros(size(basis,3),2);
    for i = size(basis,3) % Get the weights
        base = reshape(basis(:,:,i), [size(basis(:,:,i),1)*size(basis(:,:,i),2), 1]);
        W(i,:) = base' * (del_I*warp_jacobian);
    end
    A = reshape(basis, [size(basis,1)*size(basis,2), size(basis,3)]);
    SDQ = (del_I*warp_jacobian) - (W'*A')';
    % Compute the Hessian
    H = SDQ'*SDQ;
